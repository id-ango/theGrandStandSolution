...
namespace GlobalSoft.Controllers
{
    ...
    public class LaporanController : Controller
    {
        ...
        public ActionResult LapSourceCode()
        {
            List<GlAccount> SourceCode = new List<GlAccount>();

        var source =  (from e in db.AptTrsNoes
             join y in db.GlAccounts
             on e.GlAkunID equals y.GlAkunID
             select new { e.TransNo, y.GlAkun, y.GlAkunName }).ToList();

            foreach (var t in source)
            {
                SourceCode.Add(new GlAccount
                {
                    GlAkun2 = t.TransNo,
                    GlAkun = t.GlAkun,
                    GlAkunName = t.GlAkunName
        ...
        public ActionResult Cetak(DateTime Tanggal1, DateTime Tanggal2, int KodeBank)
        {
            ...
            var sp = (from b in db.ArTransDs
                      join y in db.ArTransHs on b.ArHGd equals y.ArHGd
                      join r in db.ArCustomers on b.CustomerID equals r.CustomerID
                      where y.BankID == id && (Tanggal1 <= y.Tanggal && y.Tanggal <= Tanggal2)
                      select new { y.Tanggal, b.Keterangan, b.SPesananID, b.Bayar, y.Bukti,r.CustomerName,b.Duedate }).ToList();
            ...
            foreach (var t in sp)
            {
                Transaksi.Add(new CbTrans
                {
                    ...
                    NoRef = t.Bukti,
                    Tanggal = t.Tanggal,
                    TglSelesai = (DateTime)t.Duedate,
                    Keterangan = t.Keterangan.Trim() +' '+t.CustomerName.Trim(),
